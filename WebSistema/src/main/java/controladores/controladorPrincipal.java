          ...
          request.getRequestDispatcher( "/mantenimientos/listadoAreas.jsp" ).forward( 
               request,response);
                ...
                if (objDB.validarUsuario(xnom, xcla) != null){
                    ...
                    response.sendRedirect("/WebSistema/sesiones/login.jsp");                          
            } 
            
            else if (accion.compareTo("CERRAR_SESION") == 0) {
                HttpSession misession= (HttpSession) request.getSession();
                misession.removeAttribute("usuario");
                misession.invalidate();
                out.print("<center><div class='container'>");
            ...
            request.getRequestDispatcher( "/mantenimientos/editarAreas.jsp" ).forward( 
             request,response );
                ...
                if (request.getParameter("boton").compareTo("GRABAR") == 0){
                    String operacion = request.getParameter("operacion");
                    String strDateI = request.getParameter("xfecI");
                    Date xfecI = Date.valueOf(strDateI);
                    String strDateF = request.getParameter("xfecF");
                    Date xfecF = Date.valueOf(strDateF);
                    Cursos curso = new Cursos();
                ...
                ICursosDAO dao = new CursoDAOImpl();
                double total = 0;
                for (String xcodCurso : xcodCursos) {
                    Cursos curso = new Cursos();
                    curso = dao.buscar(Integer.parseInt(xcodCurso));
                ...
                if(request.getParameter("modo").compareTo("Lista") == 0){
                    ...
                    List<Matriculas> arrMatriculas = new ArrayList<Matriculas>();
                    Matriculas matricula = new Matriculas();
                    matricula.setNro_doc(request.getParameter("xcod"));
                    IMatriculaDAO dao = new MatriculaDAOImpl();
                    arrMatriculas = dao.buscarMatriculas(matricula);
                    request.setAttribute("arrMatriculas", arrMatriculas);
                    request.setAttribute("codigo", matricula.getNro_doc());
                    request.getRequestDispatcher("/operaciones/matriculaMostrarMatriculas.jsp").forward(request, response);
                }
            }
            else{
